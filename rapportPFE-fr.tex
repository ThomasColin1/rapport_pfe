...
Cette page principale comporte un menu de côté dépliable, qui montre la liste des modules qu'on afficher dans l'application. En cliquant dessus, on rend visible l'iframe concernée, et on affiche ainsi l'interface sélectionnée.

Ce menu de côté est rempli au démarrage de l'application : la page principale requête le serveur par rapport à quels sont les end-points où une ou plusieurs interfaces sont disponibles. Ensuite, elle requête le fichier de configuration de ces interfaces Web, et y trouve ses interfaces, leurs logos, leurs noms et leurs URLs correspondantes. Enfin, elle affiche une carte dans le menu de côté pour chaque interface, pêrmettant ainsi à l'utilisateur de naviguer facilement entre les modules.
...
Tous les paramètres du MiniLab3 sont décrit dans le fichier JSON décrivant l'appareil, avec leur nom, si ce sont des paramètres généraux ou liés à un contrôle en particulier, ainsi que leur valeur par défaut. 

Les paramètres du MiniLab3 sont regroupés en deux catégories :
\begin{itemize}
    \item Les paramètres d'appareils sont des paramètres généraux, qui comportent par exemple le temps de mise en veille, ou le mode de fonctionnement de la pédale. Ces paramètres sont récupérés au branchement de l'appareil, et sont stockés dans un store. Ils sont mis à jour à chaque changement : dès que l'utilisateur modifie un paramètre, un message SysEx est envoyé à l'appareil. Ils sont regroupés par catégories.
    \item Les paramètres de préréglage sont des paramètres assignés à chaque contrôle de l'appareil. Par exemple, cela peut être la note assignée à un pad, ou le minimum et le maximum d'un slider. 
    
    Ceux-ci sont assignés à des "profils" sur le MiniLab3 : il y a plusieurs profils non modifiables, comme par exemple le profil DAW qui est compatible avec les logiciels de Musique Assistée par Ordinateur, mais aussi 6 profils modifiables. Ainsi, on importer peut le profil que l'on désire depuis le MiniLab3, le modifier, puis le renvoyer sur le profil que l'on souhaite sur l'appareil. 
    
    On peut également importer et exporter ces paramètres sur des fichiers locaux sur l'ordinateur. Ceci permet à l'utilisateur de sauvegarder un paramètrage de son clavier, ou encore le partager sur une autre unité. La sauvegarde à un endroit spécifique de l'ordinateur est impossible en Web, car elle pose des problèmes de sécurité. Ainsi, nous avons théorisé, avec l'équipe Compagnon, un module sans interface Web, qui servirait de FileSystem. Il aurait un end-point pour importer un fichier, et un autre pour exporter. Créer ce module sans interface plutôt qu'intégrer cette fonctionnalité au back-end du MCC MiniLab3 permettrait de créer un module générique utilisable par tout autre module Web ayant besoin du FileSystem. En attendant le développement d'un tel service, j'ai réalisé une importation par selection de fichier, et une exportation en téléchargement dans le dossier par défaut de téléchargement du navigateur.
\end{itemize}
...
Cette méthode d'envoi séquentiel de SysEx, en attente de réponses avant d'envoyer les suivants, convient au MiniLab3 en raison de son nombre limité de paramètres. Pour un appareil avec plus de paramètres, nous pouvons également envisager d'envoyer tous les SysEx à la suite sans attendre les réponses, et de traiter les réponses à leur réception.
...
J'en ai repris les couleurs et les polices de charactères. Je me suis également inspiré des cartes avec ombres de l'ASC pour les différents menus du Midi Control Center.
